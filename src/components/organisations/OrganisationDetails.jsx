...
export default function OrganisationDetails({ organisation, onClose, onEdit, onDelete }) {
  ...
  const { data: customers = [], isLoading: customersLoading } = useQuery({
    ...
    queryFn: async () => {
      // Fetch all customers (Base44 relationships are one-way)
      const allCustomers = await base44.entities.Customer.list();
      
      // Filter out deleted customers and match by organisation_id
      const result = allCustomers.filter(c => 
        !c.deleted_at && c.organisation_id === organisation.id
      );
