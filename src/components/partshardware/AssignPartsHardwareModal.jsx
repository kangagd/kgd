...
export default function AssignPartsHardwareModal({ open, onClose, item, vehicles }) {
  ...
  const assignMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast.success("Item assigned to selected vehicles");
      onClose();
      setSelectedVehicles([]);
    },
    onError: (error) => {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-md">
        <DialogHeader>
          <DialogTitle>Assign "{item.name}" to Vehicles</DialogTitle>
        </DialogHeader>
