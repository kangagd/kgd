...
export default function PriceListModal({ open, onClose }) {
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-4xl max-h-[85vh] border-2 border-slate-200 rounded-2xl">
        ...
        <div className="space-y-4">
          <div className="relative">
            ...
            <Input
              ...
              className="pl-11 h-12 border-2 border-slate-300 focus:border-[#fae008] focus:ring-2 focus:ring-[#fae008]/20 rounded-xl font-medium"
            />
          </div>

          <Tabs value={categoryFilter} onValueChange={setCategoryFilter}>
            <TabsList className="w-full h-11 bg-slate-100 p-1">
            ...
            <div className="space-y-2.5">
              {filteredItems.length === 0 ? (
                ...
                filteredItems.map((item) => (
                  <div
                    key={item.id}
                    className="p-4 bg-white border-2 border-slate-200 rounded-xl hover:border-[#fae008] hover:shadow-md transition-all"
                  >
