  ...
  return (
    <div className="space-y-4">
      ...
      {unassignedJobs.length > 0 && (
        <div>
          <h3 className="text-sm font-bold text-[#4F4F4F] uppercase tracking-wide mb-3 px-1">
            ...
            {unassignedJobs.map(job => (
              ...
              >
                <CardContent className="p-4">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1 flex-wrap">
                        <span className="text-sm font-bold text-[#111111]">#{job.job_number}</span>
                        <Badge className={`${statusColors[job.status] || ''} font-semibold text-xs border-2`}>
                          {job.status.replace(/_/g, ' ')}
                        </Badge>
                      </div>
                      <div className="font-semibold text-base text-[#111111] mb-1">
                        {job.customer_name}
                      </div>
                    </div>
                    {job.scheduled_time && (
                      <div className="flex items-center gap-1 text-sm font-bold text-[#111111] bg-[#FEF8C8] px-2 py-1 rounded-lg">
                  ...
                  {job.address && (
                    <div className="flex items-start gap-2 text-sm text-[#4F4F4F] mb-2">
        ...
        return (
          <div key={technician.id}>
            <div className="flex items-center gap-2 mb-3 px-1">
              <div className="w-8 h-8 bg-[#FAE008] rounded-full flex items-center justify-center">
                ...
                >
                  <CardContent className="p-4">
                    <div className="flex items-start justify-between gap-2 mb-2">
                      ...
                      {job.scheduled_time && (
                        <div className="flex items-center gap-1 text-sm font-bold text-[#111111] bg-[#FEF8C8] px-2 py-1 rounded-lg flex-shrink-0">
                          <Clock className="w-3 h-3" />
                          {job.scheduled_time.slice(0, 5)}
                        </div>
                      )}
      ...
      {dayJobs.length === 0 && (
        <Card>
          <CardContent className="p-12 text-center">
            <Calendar className="w-12 h-12 mx-auto text-[#E2E3E5] mb-3" />
            <h3 className="text-base font-semibold text-[#111111] mb-1">No jobs scheduled</h3>
            <p className="text-sm text-[#4F4F4F] mb-4">No jobs found for this day</p>
            <Button
