            ...
            <Card className="bg-slate-50 border-slate-200">
              <CardContent className="p-6">
                ...
                <div className="text-3xl font-bold text-slate-900">
                  ${(dashboardData?.total_balance || 0).toFixed(2)}
                </div>
                <div className="text-xs text-slate-500 mt-1">
                  {dashboardData?.invoice_count || 0} invoice{dashboardData?.invoice_count !== 1 ? 's' : ''}
                </div>
              </CardContent>
            </Card>
            <Card className="bg-amber-50 border-amber-200">
              <CardContent className="p-6">
                <div className="text-sm font-medium text-amber-600 mb-1">Total Owing</div>
                <div className="text-3xl font-bold text-amber-900">
                  ${(dashboardData?.total_owing || 0).toFixed(2)}
                </div>
                <div className="text-xs text-amber-700 mt-1">Unpaid amounts</div>
              </CardContent>
            </Card>
            <Card className="bg-emerald-50 border-emerald-200">
              <CardContent className="p-6">
                <div className="text-sm font-medium text-emerald-600 mb-1">Total Paid</div>
                <div className="text-3xl font-bold text-emerald-900">
                  ${((dashboardData?.total_balance || 0) - (dashboardData?.total_owing || 0)).toFixed(2)}
                </div>
                <div className="text-xs text-emerald-700 mt-1">
                  {dashboardData?.total_balance > 0 
                    ? `${(((dashboardData.total_balance - dashboardData.total_owing) / dashboardData.total_balance) * 100).toFixed(0)}% collected`
                    : 'No invoices'
                  }
                </div>
              </CardContent>
