...
export default function LinkEmailThreadModalContract({ open, onClose, contractId, onLink, isLinking }) {
  const [searchTerm, setSearchTerm] = useState("");

  // Fetch threads linked to this contract
  const { data: linkedThreads = [] } = useQuery({
    queryKey: ['contractEmailThreads', contractId],
    queryFn: () => base44.entities.EmailThread.filter({ contract_id: contractId }),
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="max-w-2xl max-h-[80vh] flex flex-col">
        <DialogHeader>
          <DialogTitle>Link Email Thread to Contract</DialogTitle>
          <DialogDescription>
            Search and select an email thread to link to this contract
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="flex-1 overflow-y-auto space-y-2 min-h-[300px]">
          {filteredThreads.length === 0 ? (
            ...
            filteredThreads.map(thread => {
              ...
              const isLinkedElsewhere = thread.contract_id && thread.contract_id !== contractId;
              const isLinkedToProject = !!thread.project_id;
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between gap-3 mb-2">
                    ...
                    {isLinkedElsewhere && (
                      <Badge className="bg-amber-100 text-amber-700 flex-shrink-0">Other Contract</Badge>
                    )}
                    {isLinkedToProject && (
                      <Badge className="bg-purple-100 text-purple-700 flex-shrink-0">Project</Badge>
                    )}
                  </div>
                  ...
                  {isLinkedElsewhere && thread.contract_name && (
                    <div className="text-[11px] text-amber-700 mt-1">
                      Currently linked to: {thread.contract_name}
                    </div>
                  )}
                  
                  {isLinkedToProject && thread.project_title && (
                    <div className="text-[11px] text-purple-700 mt-1">
                      Currently linked to project: {thread.project_title}
                    </div>
                  )}
