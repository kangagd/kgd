...
Deno.serve(async (req) => {
  try {
    ...
    if (allMessages.length === 0) {
      return Response.json({ synced: 0, message: 'No messages to sync', version: '2026-01-29' });
    }
    ...
    async function processMessage(message) {
      try {
        ...
        // CRITICAL: Use centralized subject decoder (DO NOT REMOVE - prevents mojibake)
        const subject = decodeEmailSubject(headers.find(h => h.name === 'Subject')?.value);
    ...
    return Response.json({ synced: syncedCount, total: queue.length, version: '2026-01-29' });
  } catch (error) {
