...
Deno.serve(async (req) => {
  try {
    ...
    if (user.role !== 'admin' && user.role !== 'manager') {
      return Response.json({ error: 'Only admins and managers can link quotes' }, { status: 403 });
    }
    ...
    if (projectId) {
      ...
      if (projects[0]) {
        ...
        organisationId = projects[0].organisation_id;
    ...
    if (jobId) {
      ...
      if (jobs[0]) {
        ...
        organisationId = organisationId || jobs[0].organisation_id;
    ...
    console.error('linkExistingPandaDocQuote error:', error);
