...
Deno.serve(async (req) => {
  try {
    ...
    // Only admins/managers can create quotes
    if (user.role !== 'admin' && user.role !== 'manager') {
      return Response.json({ error: 'Only admins and managers can create quotes' }, { status: 403 });
    }
    ...
    if (jobId) {
      ...
      organisationId = organisationId || job.organisation_id;
    }
    ...
    const quote = await base44.entities.Quote.create({
      ...
      pandadoc_public_url: '', // Will be set when document is sent/viewed
    ...
    console.error('createPandaDocQuoteFromProjectOrJob error:', error);
